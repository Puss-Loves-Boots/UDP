    ...
    , ui(new Ui::Widget)
{
    ...
    QString size = QString("%1 字节").arg(QString::number((ui->sendEdit->toPlainText()).size()));
    ui->send_size->setText(size);
    connect(ui->sendEdit, &QTextEdit::textChanged, this, [=]()
    {
        QString size = QString("%1 字节").arg(QString::number((ui->sendEdit->toPlainText()).size()));
        ui->send_size->setText(size);
    });
...
///////////////////////////////循环发送功能///////////////////////////////////////////
    ...
    {
        int looptime = (ui->looptime->text()).toInt();
        // 启动定时器
        if(timer->isActive())
        {
            timer->stop();  // 关闭定时器
            ui->loopBtn->setText("循环发送");
        }
        else
        {
            ui->loopBtn->setText("取消循环发送");
            timer->start(looptime); // ms
        }
    });
...
void Widget::on_sendBtn_clicked()
{
    ...
    sendBuff = ui->sendEdit->toPlainText();                //发送的数据报
...
void Widget::on_clearBtn_send_clicked()
{
    ui->sendEdit->clear();
}
